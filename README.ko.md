...
첫 화면은 짧습니다. 제목, 영상 놓기, **봇에게 이 말 복사**. 그 저장은 봇 하나입니다(`source_mode: bot`). Cursor 에이전트는 같은 PC면 체크인하고, 다른 컴퓨터면 초대문의 편집 인박스 폴더에만 둡니다. 스타일 레시피와 봇 둘 규격은 **더 자세히** 뒤에 있습니다. 편집 문으로 온 컷은 받기 버튼 없이 열립니다. 이 책상은 사이트를 긁지 않습니다.

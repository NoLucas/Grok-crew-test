...
```
내 규격  →  Grok 문 또는 다른 에이전트 문  →  그 문 인박스  →  이 PC가 받음  →  내가 원할 때만 게시
```
...
첫 화면에는 **문이 두 개**입니다. Grok 문 또는 다른 에이전트 문에 규격을 적고, 그 글만 그 도우미에게 주세요. Grok 패키지는 `handoff-inbox/grok`, Claude·Codex·ChatGPT는 `handoff-inbox/agents`에 둡니다. **받기**는 자기 문만 가져옵니다. **이 문으로 예시 도착 보기**는 같은 도착을 번들 클립으로 보여 줍니다.

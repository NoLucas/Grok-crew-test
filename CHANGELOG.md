...
- Crew role names are always `Editor Agent` and `Collector Agent`. Grok, Claude, and other brand aliases are not stored as the role. Folder names stay `handoff-outbox/grok` and `handoff-outbox/agents`. Desktop reads `crew_roster` from `/api/v2/workspace`.

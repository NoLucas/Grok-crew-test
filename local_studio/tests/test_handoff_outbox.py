...
def test_create_spec_writes_only_that_door_outbox(studio):
    ...
    grok_folder = _outbox_path("editor", grok["id"])
    agent_folder = _outbox_path("collector", agent["id"])
    ...
    assert payload["door"] == "editor"
    assert payload["agent"] == "Editor Agent"
    assert payload["return"]["inbox"].endswith("handoff-inbox/editor/")
    assert payload["return"]["git_prefix"] == "editor/"
    assert "will not attach footage" in brief
    assert agent_folder.is_dir()
    assert not (config.WORKSPACE_DIR / "handoff-outbox" / "editor" / agent["id"]).exists()
    assert not (config.WORKSPACE_DIR / "handoff-outbox" / "collector" / grok["id"]).exists()

    status = outbox_status()
    grok_ids = {item["id"] for item in status["doors"]["editor"]["pending"]}
    agent_ids = {item["id"] for item in status["doors"]["collector"]["pending"]}
    assert grok["id"] in grok_ids
    assert agent["id"] not in grok_ids
    assert agent["id"] in agent_ids
    assert grok["id"] not in agent_ids
    assert status["doors"]["collector"]["pending"][0]["agent"] == "Collector Agent"
...
def test_attach_spec_project_archives_outbox(studio, tmp_path):
    ...
    folder = _outbox_path("editor", record["id"])
    ...
    processed = config.WORKSPACE_DIR / "handoff-outbox" / "editor" / ".processed" / record["id"]
    assert processed.is_dir()
    assert (processed / "spec.json").is_file()
    status = outbox_status()
    assert record["id"] not in {item["id"] for item in status["doors"]["editor"]["pending"]}
...
def test_attach_without_project_media_still_archives(studio):
    ...
    assert not folder.exists()
    assert (config.WORKSPACE_DIR / "handoff-outbox" / "editor" / ".processed" / record["id"] / "spec.json").is_file()
...
def test_push_outbox_skips_without_remote(studio, monkeypatch):
    ...
    assert _outbox_path("editor", record["id"]).is_dir()

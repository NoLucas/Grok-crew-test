...
```
仕様  →  収集送信箱 + 編集送信箱  →  収集ボットが素材箱にクリップを置く  →  Grok がそのクリップだけ切る  →  編集受信箱  →  このPCが受け取る  →  自分が言うときだけ投稿
```
...
最初の画面は**ボット二人で一つの仕様**を引き継ぎます。収集ボット（Claude・Codex・ChatGPT など）は `handoff-outbox/agents` を読み、クリップを `handoff-materials` に置きます。Grok は `handoff-outbox/grok` を読み、そのクリップだけ切って `handoff-inbox/grok` に返します。このデスクはサイトを掻きません。文のコピーは予備です。**収集の例を見る** と **編集の届き方を見る** は同梱クリップで同じ到着を見せます。

...
```
仕様  →  Grok ドアまたは他エージェントドア  →  そのドアの受信箱  →  このPCが受け取る  →  自分が言うときだけ投稿
```
...
最初の画面には**ドアが二つ**あります。Grok ドアか他エージェントドアに仕様を書き、その文だけをその相手に渡します。Grok のパッケージは `handoff-inbox/grok`、Claude・Codex・ChatGPT は `handoff-inbox/agents` に置きます。**受け取る** は自分のドアだけ取り込みます。**このドアの届き方を見る** は同梱クリップで同じ到着を見せます。

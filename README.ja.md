...
**Grok Crew は、あなたのパソコンにあるショート動画デスクです。仕様は人が書き、素材と編集は Grok ボットが渡します。**

このパソコンに原版を置かなくても大丈夫です。長さ、字幕、残すセリフだけ決めます。**別のパソコン**のボットが映像とカットを作り、フォルダで渡します。この PC は受け取ります。Instagram・TikTok・YouTube は、したいときだけ投稿します。

素材を預けるサイトではありません。ボットはこの PC を開かず、パッケージだけ送ります。

```
仕様  →  ボットが素材とカットを作る  →  このPCが受け取る  →  自分が言うときだけ投稿
```

## だれ向けか

- リールの仕様だけ決めたい人
- 別のパソコンの Grok に素材と初回カットを任せたい人
...
最初の画面で**仕様**を書きます。保存した文をコピーして、別のパソコンの Grok に渡します。ボットがパッケージを送ったら **ボットの編集を受け取る** を押します。**届き方の例を見る** は同梱クリップで同じ到着を見せます。
